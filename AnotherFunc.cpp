...
int main()
{
    ...
    int a[n][n]; /// \brief int a[n][n] - Матрица заданной размерности.
    random_device rand; /// \brief random_device rand - Вызов рандомизации.
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            a[i][j]=Rand(); /// \brief a[i][j]=Rand() - Заполнение матрицы возвращённым случайным значением из функции.
        }
    }
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            cout << a[i][j]<<" "; /// \brief cout << a[i][j]<<" " - Вывод матрицы.
        }
        cout << endl;
    }
    while (Win(a) == true) { /// \brief while (Win(a) == true) - Цикл с условием победы.
        int str, sto;
        ...
        str--;
        sto--;
        if (str >= 0 && str < n && sto >= 0 && sto < n) { /// \brief if (str >= 0 && str < n && sto >= 0 && sto < n) - Проверка входа введённого значения в диапазон.
            if (a[str][sto] == 1) { /// \brief if (a[str][sto] == 1) - Проверка значения в ячейке матрицы.
                a[str][sto] = 0; /// \brief a[str][sto] = 0 - Ввод противоположного значения.
            }
            else { /// \brief else - Проверка значения в ячейке матрицы.
                a[str][sto] = 1; /// \brief a[str][sto] = 1 - Ввод противоположного значения.
            }
            cout << "Изменённая матрица: \n";
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
                    cout << a[i][j] << " "; /// \brief cout << a[i][j] << " " - Вывод изменённой матрицы.
                }
                cout << endl;
            }
        }
